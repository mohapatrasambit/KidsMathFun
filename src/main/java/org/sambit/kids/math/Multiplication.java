...
public class Multiplication implements Equations{
    /**
    ...
    public List<String> generateEquations(Integer numberOfEquations, Integer ceiling) {
        ...
        for (var idx = 1; idx <= numberOfEquations; idx++) {
            if(idx % 3 == 0) {
                listOfEquations.add(String.format("%d x %d = \n", random.nextInt(ceiling) + 1, random.nextInt(ceiling)));
            } else {
                listOfEquations.add(String.format("%d x %d = \t\t\t", random.nextInt(ceiling) + 1, random.nextInt(ceiling)));
            }
        }
